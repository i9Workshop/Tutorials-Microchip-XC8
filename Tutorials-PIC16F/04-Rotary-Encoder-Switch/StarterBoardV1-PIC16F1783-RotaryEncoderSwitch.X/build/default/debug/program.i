...
void programLoop(void) {
    ...
    while(1) {
        ...
        encoderData = ((PORTCbits.RC0<<1) | PORTCbits.RC2) & 0x03;

        if(encoderData==0x03) {

        }

        else if(encoderData==0x02) {
            if(counter<60000) counter++;

            lcd_Goto(1, 0);
            lcd_PrintDigitInt32(counter, 5, 0, 1);
